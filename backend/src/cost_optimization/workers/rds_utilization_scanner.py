"""Lambda entry point for persisted RDS utilization recommendations."""
...
from cost_optimization.infrastructure.aws.rds_instances import (
    Boto3RdsInstanceDiscovery,
    create_rds_client,
)
...
_SCANNER_NAME = "sustained-low-utilization-rds-instance"
...
def handler(event: Mapping[str, object], context: LambdaContext) -> dict[str, object]:
    """Run a persisted RDS utilization scan in response to an invocation."""
    del event
...
def run_scan(
    ...
    """Compose and execute the RDS recommendation workflow with explicit dependencies."""
    ...
    logger.info("rds_utilization_scan_completed", extra={"scan_id": completed_scan.scan_id})
    return _scan_summary(completed_scan)


def build_workflow(settings: Settings, account_id: str) -> RunDetectionScan:
    """Wire RDS and CloudWatch readers into a read-only recommendation scan."""
    region, findings_table_name, scan_runs_table_name = settings.require_scanner_configuration()
    detector = DetectIdleRdsInstances(
        Boto3RdsInstanceDiscovery(create_rds_client(region), account_id=account_id, region=region),
        Boto3CloudWatchMetricReader(create_cloudwatch_client(region)),
        IdleRdsInstanceRule(
            IdleRdsInstanceRuleConfig(
                lookback_days=settings.utilization_lookback_days,
                maximum_cpu_percent=settings.rds_maximum_cpu_percent,
            )
        ),
        lookback_days=settings.utilization_lookback_days,
    )
