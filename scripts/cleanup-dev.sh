#!/usr/bin/env bash
# Deletes the development deployment and SAM-managed artifacts.
#
...
Deletes the aws-cost-optimizer-dev SAM application and the deployment artifacts
created for it. The Vercel dashboard is a separate provider resource and is not
deleted by this AWS-only script. The script never targets staging or production.
...
echo "Region: $region"
echo "This permanently deletes the '$STACK_NAME' development stack and SAM artifacts."
...
echo "If created, delete the separate Vercel dashboard project through Vercel before closing the environment."
